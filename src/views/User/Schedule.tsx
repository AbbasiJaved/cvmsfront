...
import { EditMeetingModal } from "../../components";
...
export const Schedule = () => {
  ...
  const filteredMeetings = meetings.value?.filter((meeting) => {
    if (meeting.course?.lecturerId !== auth.user?.id) {
      return false;
    }
    ...
    if (meeting.timetable?.viewable === false) {
      return false;
    }
      ...
      <Spin spinning={meetings.loading}>
        <div>
          ...
          >
            ...
            >
              {courses.value
                ?.filter((course) => course.lecturerId === auth.user?.id)
                .map((course) => (
          ...
          {events.length > 0 ? (
            <FullCalendar
              ...
              ref={calendarRef}
              events={events}
              eventContent={renderEventContent({ onDelete: undefined })}
              validRange={{
                start: startDate,
                end: endDate,
              }}
